...
const AdminPage = () => {
 ...
 const loadBookings = async (filters = bookingFilters) => {
  ...
  // Validation helpers for showtime
  const getMinShowtimeDate = () => {
    const now = new Date();
    now.setHours(now.getHours() + 3); // 3 hours minimum
    return now;
  };
  ...
  const handleSubmitShowtime = async (event: FormEvent) => {
    event.preventDefault();
    try {
      // Validate showtime (only for new showtimes, not for edits)
      if (!showtimeForm.id && !isValidShowtime(showtimeForm.start_time)) {
        notification.error({
          message: "Thời gian không hợp lệ",
          description: "Suất chiếu phải bắt đầu sau thời điểm hiện tại ít nhất 3 tiếng.",
        });
        return;
      }
